...
test('PUT /posts/id', async () =>{
    ...
    const response = await updatePost(1,updatePOst)

    expect(response.status).toBe(200)
    expect(response.data).toHaveProperty('id')
    expect(response.data).toHaveProperty('title')
    expect(response.data.title).toBe(updatePOst.title)
    expect(response.data.body).toBe(updatePOst.body)
    expect(response.data.userId).toBe(updatePOst.userId)
